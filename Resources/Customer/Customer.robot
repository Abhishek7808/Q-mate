...
Fill CRO
    ...
    ${List Item}    Run Keyword If    ${Status}    Get List Items    purchaseOrderId
    ...
    #Sleep    2s
    #Press Key    ${Key Description["Contract Release Order Amount"]["Locator"]}    \\08
    Input Valid Value    Contract Release Order Amount    ${CRO["Amount"]}
    Input Valid Value    Contract Release Order Button
...
Approve FI
    ...
    Sleep    3s
    ${Status}    Run Keyword And Return Status    Page Should Contain Element    //td[contains(text(),'${Branch["Name"]},${Company["Company Name"]}')]/following-sibling::td/i[contains(text(),'visibility')]
    Run Keyword If    '${Status}'=='False'    Click Button    Next
    Sleep    2s
    Click Element    //td[contains(text(),'${Branch["Name"]},${Company["Company Name"]}')]/following-sibling::td/i[contains(text(),'visibility')]
    Sleep    3s
...
Approve PO
    ...
    Click Button    btnApplytFillter
    Sleep    3s
    ${PO No}    Get Text    //td[contains(text(),'${Branch["Name"]},${Company["Company Name"]}')]/ancestor::tr/td[2]
    [Return]    ${PO No}
...
Fill Outward Slip
    ...
    Sleep    1s
    Input Text Value    ${Key Description["Search In Vehicle"]["Locator"]}    ${CRO No}
    Set To Dictionary    ${Key Description["Generate Out Slip"]}    Locator=//td[contains(text(),'${Dispatch["Enter Vehicle Number"]}')]/following-sibling::td/button
    Sleep    4s
